...
class AppointmentRepo {
    ...
    async updatePatientId(appointmentId, patientId) {
        try {
            const updatedAppointment = await AppointmentModel.findByIdAndUpdate({ _id: appointmentId }, { patientId }, { new: true });
            if (!updatedAppointment)
                throw new Error("No appointment found with the provided ID.");
            return updatedAppointment;
        }
        catch (error) {
            throw new Error("Error updating appointment with patientId: " + error.message);
        }
    }
    async findAppointment(doctorId, appointmentDate, timeSlot) {
    ...
    async appointmentRetrievalWithPatient(appointmentId) {
        try {
    ...
    async paymentStatusChange(appointmentId) {
        try {
            const changedPaymentStatus = await AppointmentModel.findByIdAndUpdate(appointmentId, { status: "confirmed", paymentStatus: "paid" }, { new: true });
    ...
    async appointmentFetching(userId) {
        try {
    ...
    async cancelAppointment(appointmentId) {
        try {
            const cancelled = await AppointmentModel.findByIdAndUpdate(appointmentId, { status: "cancelled" }, { new: true });
            if (!cancelled) {
    ...
    async fetchingOnlineAppointmentRepo(userId) {
        try {
    ...
    async fetchingOfflineAppointments(userId) {
        try {
            ...
            throw new Error("Error occurred while fetching offline appointments from the database.");
        }
    }
    async appointmentListRepo(doctorId) {
        try {
    ...
    async fetchMonthlyAppointmentData() {
        try {
            const data = await AppointmentModel.aggregate([
                {
                    $match: { status: { $in: ["confirmed", "completed"] } },
    ...
    async fetchDailyAppointmentData() {
        try {
            const data = await AppointmentModel.aggregate([
                {
                    $match: { status: { $in: ["confirmed", "completed"] } },
    ...
    async fetchingspecialisationStats() {
        try {
            const data = await AppointmentModel.aggregate([
                {
                    $match: {
            ...
            const totalAppointments = data.reduce((sum, item) => sum + item.totalAppointments, 0);
            const formattedData = data.map((item) => ({
                specialization: item._id,
                totalAppointments: item.totalAppointments,
                percentage: ((item.totalAppointments / totalAppointments) *
                    100).toFixed(2),
            }));
            return formattedData;
        }
    ...
    async getSpecialisationPercentageRepo() {
        try {
    ...
    async getAppointmentForDashboard() {
        try {
            const data = await AppointmentModel.aggregate([
                {
                    $match: {
    ...
    async countAppointmentsForToday(doctorId) {
        try {
    ...
    async getTotalAppointmentsCount(doctorId) {
        try {
    ...
    async getTotalOnlineAppointmentsForDoctor(doctorId) {
        try {
            const result = await AppointmentModel.aggregate([
                {
                    $match: {
    ...
    async getTotalOfflineAppointmentsForDoctor(doctorId) {
        try {
            const result = await AppointmentModel.aggregate([
                {
                    $match: {
    ...
    async getLatestAppointmentRepo(doctorId) {
        try {
            const appointments = await AppointmentModel.aggregate([
                {
                    $match: {
    ...
    async getTotalPatientCountFromAppointments(doctorId) {
        try {
    ...
    async markAsCompleted(appointmentId) {
        try {
            const updatedStatus = await AppointmentModel.findByIdAndUpdate(appointmentId, { status: "completed" }, { new: true });
            if (!updatedStatus) {
                throw new Error("Appointment not found");
            }
