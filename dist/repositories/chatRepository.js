...
class ChatRepo {
    ...
    async fetchChatRepository(userId) {
        try {
            ...
            const fieldsToSelect = modelToPopulate === "Doctor"
            ...
            await Promise.all(fetchedChats.map(async (chat) => {
    ...
    async fetchChatByIdRepo(chatId) {
        try {
    ...
    async lastMessageUpdateRepo(chatId, senderId) {
        try {
            const updatedChat = await Chat.findByIdAndUpdate({ _id: chatId }, { lastMessage: senderId }, { new: true }).lean(); // Use `.lean()` for better performance if needed
            return updatedChat;
        }
        catch (error) {
