...
const generateRefreshToken = async (Id: string, role: string): Promise<string> => {
    try {
      // Create a new JWT token for the refresh token
      const token = jwt.sign({ Id, role }, process.env.JWT_REFRESH_TOKEN as string, { expiresIn: '1d' });
